...
export default function RemoveAccess(props) {
  function removeInstructor(e) {
    e.preventDefault();
    database.users.doc(props.email).update({
      instructor: false,
    }).then(() => {
      console.log('Instructor successfully removed!')
      props.setOpen(false);
    });
  }
              ...
              <div>
                ...
                <div className="mt-3 text-center sm:mt-5">
                  <Dialog.Title as="h3" className="text-lg leading-6 font-medium text-gray-900">
                    Remove instructor access
                  </Dialog.Title>
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      Are you sure you want to remove instructor access from <span className="font-bold">{props.email}</span>?
                    </p>
                  </div>
                </div>
              </div>

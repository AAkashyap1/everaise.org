...
export default function Question(props) {
  ...
  useEffect(() => {
    if (!['MC', 'SA', 'FR'].includes(type) && type !== '') {
      setError(
        'The question type must be MC (multiple choice), SA (short answer), or FR (free response)'
      );
    } else {
      setError('');
    }
            ...
            <div className="space-y-6 lg:px-0 lg:col-span-9">
              <form onSubmit={HandleSubmit} action="#" method="POST">
                <div className="shadow sm:rounded-md overflow-hidden shadow border border-gray-200">
                  {error && (
                    <div className="rounded-md bg-yellow-50 p-4 mx-3 mt-3">
                      <div className="flex">
                  ...
                  <div className="px-4 py-3 bg-gray-50 text-right sm:px-6">
                    <button
                      ...
                      className={
                        created
                          ? 'bg-cyan-300 border border-transparent rounded-md shadow-sm py-2 px-4 inline-flex justify-center text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500'

...
export default function EditModuleModal(props) {
  ...
  async function changeDocuments() {
    try {
      setLoading(true);
      let tempAssignments = [];
      const assignmentsRef = props.data.doc(props.id).collection('assignments');
      const assignmentData = await assignmentsRef.get();
      let i = 0;

      assignmentData.docs.forEach((doc) => {
        tempAssignments.push({
          id: assignmentData.docs[i].id,

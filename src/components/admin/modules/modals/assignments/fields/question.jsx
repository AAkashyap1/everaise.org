...
export default function Question(props) {
  ...
  useEffect(()=> {
    if (props.assignment.questions[props.index].type) {
      setType(props.assignment.questions[props.index].type === 'MC' ? types[0] : types[1]); 
    }
  }, [props])

  useEffect(() => {
    if (props.assignment.questions[props.index].type === 'MC') {
      if (!['A', 'B', 'C', 'D', 'E'].includes(props.assignment.questions[props.index].answer)) {
  ...
  return (
    <div className={"border-l-2 border-gray-300 pl-3 ml-1" + (props.index === 0 ? " mt-3" : " mt-4")}>
      <div className="mt-2 grid grid-cols-2 gap-2">
        ...
        <div className="col-span-2">
          <input
            ...
            placeholder="Enter solution image link"
            className="block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
          />
        </div>
      </div>
      {error && <div className="mt-3 rounded-md bg-red-50 px-3 py-2.5">
        <div className="flex">

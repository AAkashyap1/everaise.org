...
export default function EditAssignmentModal(props) {
  ...
  useEffect(() => {
    if (props.id && props.ids && props.assignments) {
      let index = 0;
      for (let i = 0; i < props.ids.length; i++) {
        if (props.ids[i] === props.id) {
          index = i;
        }
      }
      let tempAssignment = props.assignments[index];
      if (tempAssignment) {
        setAssignment({
          name: tempAssignment.name,
          disabled: tempAssignment.disabled,
          due: tempAssignment.due,
          handouts: tempAssignment.handouts,
          questions: tempAssignment.questions,
        })
      }
    }
  }, [props.id, props.ids, props.assignments])
  ...
  async function editAssignment(e) {
    ...
    try {
      ...
      await props.data.doc(`${props.name}`).collection('assignments').doc(props.id).update({
          ...
          >
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              ...
              <div>
                ...
                <form className="mt-6 grid grid-cols-2 gap-6" onSubmit={editAssignment} method="POST">

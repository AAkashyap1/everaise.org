...
export default function Timeline() {
  ...
  async function getTimeline() {
    try {
      const tempModule = await courseData[course].assignmentData.doc(module).collection('assignments').get();
      const userModule = await database.users.doc(currentUser.email)
        .collection('courses')
        .doc(course)
        .collection('modules')
        .doc(module)
      ...
      for (let i = 0; i < userModule.docs.length; i++) {
        ...
        tempTimeline.push({
          ...
          type: completed === 0 ? eventTypes.notstarted : (completed === userModule.docs[i].data().questions.length ? eventTypes.complete : eventTypes.progress),
          href: `/homework/${course}/${module}/${userModule.docs[i].id}`,
          content: completed === 0 ? 'Not Started' : (completed === userModule.docs[i].data().questions.length ? 'Complete' : 'In Progress'),
